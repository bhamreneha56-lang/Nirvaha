...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_ROLE': {
      ...
      const isObj = rawPayload && typeof rawPayload === 'object';
      const newRole = isObj ? rawPayload.role : rawPayload;
      const userName = isObj ? rawPayload.userName : undefined;
      const jurisdiction = isObj ? rawPayload.jurisdiction : undefined;
    ...
    // ------------------------------------------------------------------------
    case 'SYNC_REAL_PROBLEMS':
      // Merge real problems with mock data, ensuring no duplicates by ID
      const realProbs = action.payload || [];
      const realIds = new Set(realProbs.map(p => p.id));
      return {
        ...state,
        problems: [...realProbs, ...state.problems.filter(p => !realIds.has(p.id))]
      };
    ...
    case 'ADD_PROBLEM': {
      ...
      const newId = `JH-NEW-${Math.floor(1000 + Math.random() * 9000)}`;
      const newProblem = {
        id: p.id || newId,
...
export function SimulationProvider({ children }) {
  ...
  const { problems: realProblems } = useProblems();

  // Sync real problems from backend into the simulation state
  useEffect(() => {
    if (realProblems && realProblems.length > 0) {
      dispatch({ type: 'SYNC_REAL_PROBLEMS', payload: realProblems });
    }
  }, [realProblems]);
