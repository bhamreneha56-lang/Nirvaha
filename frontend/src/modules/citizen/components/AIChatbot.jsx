...
export default function AIChatbot() {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (lower.includes('track') || lower.includes('pid') || lower.includes('status')) {
        ...
        reply = "You earn **Karma Points** for submitting validated societal problems (+50 KP), voting on local priorities (+10 KP), and community verifications (+25 KP). These can be orangeeemed for civic badges and recognition.";
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(v => !v)}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-br from-orange-500 to-orange-500 rounded-full shadow-2xl shadow-orange-500/40 flex items-center justify-center hover:scale-110 active:scale-95 transition-all"
      ...
      >
        ...
        {/* Unread badge */}
        {!open && (
          <span className="absolute -top-1 -right-1 w-4 h-4 bg-orange-500 rounded-full flex items-center justify-center text-[9px] font-black text-white">1</span>
        )}
      </button>

      {/* Chat Drawer */}
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-h-[580px] flex flex-col bg-white rounded-2xl shadow-2xl border border-blue-100 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-4 py-3 flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-orange-400 to-orange-400 rounded-xl flex items-center justify-center text-base font-black text-white">N</div>
            <div className="flex-1">
              <div className="text-white font-black text-sm">NIRVA AI Assistant</div>
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                <span className="text-black text-[10px] font-medium">Poweorange by Gemini • Always online</span>
              </div>
            </div>
            <button onClick={() => setMessages([{ role: 'model', text: `Namaskar! 🙏 How can I help you?` }])}
              className="text-black hover:text-white text-xs font-medium transition-all">Clear</button>
          </div>

          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-white" style={{ maxHeight: '380px' }}>
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'bg-white text-black rounded-tl-sm shadow-sm border border-blue-100'
                }`}>
            ...
            {loading && (
              <div className="flex justify-start">
                ...
                <div className="bg-white rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm border border-blue-100">
          ...
          {/* Quick prompts */}
          {messages.length <= 2 && (
            <div className="px-3 py-2 bg-white border-t border-blue-100 flex gap-2 flex-wrap">
              {QUICK_PROMPTS.map(q => (
          ...
          <form onSubmit={sendMessage} className="p-3 border-t border-blue-100 flex gap-2 bg-white">
            <input
              ...
              className="flex-1 bg-white border border-blue-100 rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:border-orange-400 transition-all"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="w-10 h-10 bg-orange-500 hover:bg-orange-600 disabled:bg-white text-white rounded-xl flex items-center justify-center transition-all shrink-0"
            >
