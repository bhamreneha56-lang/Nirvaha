  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Solution Marketplace</h1>
          <p className="text-sm text-slate-500 font-medium">Discover off-the-shelf academic and industrial solutions for governance problems.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input type="text" placeholder="Search technologies..." className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-500 w-64" />
          </div>
          <button 
            onClick={() => dispatch({ type: 'ADD_TOAST', payload: { title: 'Filters', message: 'Filter panel opened.', type: 'info' } })}
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 bg-white shadow-sm hover:bg-slate-50"
          >
      ...
      <div className="grid grid-cols-3 gap-6">
        {solutions.map(s => (
          <div key={s.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow">
            ...
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{s.domain}</div>
            <h3 className="text-lg font-bold text-slate-900 mb-2">{s.title}</h3>
            <div className="text-sm font-medium text-slate-600 mb-4">Provider: {s.provider}</div>
            
            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
              <div>
                <div className="text-[10px] uppercase font-bold text-slate-400">Est. Cost</div>
                <div className="text-sm font-black text-slate-800">{s.cost}</div>
              </div>
              <button 
