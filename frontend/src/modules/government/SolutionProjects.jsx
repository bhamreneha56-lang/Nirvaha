  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-black tracking-tight">Solution Projects</h1>
          <p className="text-sm text-black font-medium">Manage the lifecycle of ongoing collaborations, pilots, and deployments.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-blue-100 flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-auto p-6">
          <div className="space-y-4">
            {projects.map(p => (
              <div key={p.id} className="border border-blue-100 rounded-xl p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <div className="text-sm text-black font-medium mt-1">Lead Partner: {p.partner}</div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black uppercase tracking-widest bg-white text-black px-3 py-1 rounded-full">
                      Stage: {p.stage}
                    </span>
                  </div>
                </div>
                ...
                <div className="mb-2 flex items-center justify-between text-xs font-bold">
                  <span className="text-black">Project Completion</span>
                  <span className={p.status === 'AT_RISK' ? 'text-orange-600' : 'text-green-600'}>{p.progress}%</span>
                </div>
                <div className="w-full h-2 bg-white rounded-full overflow-hidden mb-4">
                  <div className={`h-full rounded-full ${p.status === 'AT_RISK' ? 'bg-orange-500' : 'bg-green-500'}`} style={{ width: `${p.progress}%` }}></div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-blue-100">
                  ...
                  <button 
                    ...
                    className="px-4 py-1.5 border border-blue-100 text-black rounded text-xs font-bold hover:bg-white transition-colors"
                  >
                    View Impact Metrics
                  </button>
                </div>
