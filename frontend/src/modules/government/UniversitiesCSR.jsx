  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-black tracking-tight">Universities & CSR Partners</h1>
          <p className="text-sm text-black font-medium">Discover and collaborate with {UNIVERSITIES.length} academic institutions and {CSR_PARTNERS.length} CSR partners.</p>
        </div>
        <div className="flex items-center gap-3">
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search partners..." className="px-3 py-2 border border-blue-100 rounded-lg text-sm bg-white shadow-sm outline-none focus:border-blue-400 w-48"/>
          <button onClick={()=>dispatch({type:'ADD_TOAST',payload:{title:'Directory Exported',message:'Partner directory PDF exported.',type:'success'}})} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-600 shadow-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-4 gap-4 shrink-0">
        {[
          ...
          {l:'Academic Institutions',v:UNIVERSITIES.length,icon:<Landmark size={18} className="text-blue-600"/>,sub:'IITs, NITs, State Universities'},
          {l:'CSR Partners',v:CSR_PARTNERS.length,icon:<Building2 size={18} className="text-orange-500"/>,sub:'Corporate social responsibility'},
          ...
          <div key={k.l} className="bg-white p-5 rounded-2xl border border-blue-100 shadow-sm">
            ...
            <div className="text-3xl font-black text-black">{k.v}</div>
            <div className="text-[10px] text-black mt-1">{k.sub}</div>
          </div>
        ))}
      </div>

      <div className="flex border border-blue-100 rounded-xl overflow-hidden bg-white shrink-0 w-fit">
        {['UNIVERSITIES','CSR PARTNERS'].map(t=>(
          <button key={t} onClick={()=>setTab(t)} className={`px-6 py-2.5 text-xs font-black uppercase tracking-widest transition-colors ${tab===t?'bg-blue-600 text-white':'text-black hover:text-black'}`}>{t}</button>
        ))}
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 pb-4">
          {tab === 'UNIVERSITIES' ? uList.map(u=>(
            <div key={u.id} onClick={()=>setSel({...u,_type:'UNIVERSITY'})} className="bg-white rounded-2xl border border-blue-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer">
              ...
              <div className="p-5 pt-6 relative">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-black leading-tight">{u.name}</div>
                    <span className="text-[10px] font-bold px-1.5 py-0.5 bg-blue-500 text-blue-600 border border-blue-400 rounded">{u.type}</span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1 mb-3">{u.domains.slice(0,3).map(d=><span key={d} className="px-1.5 py-0.5 bg-white text-black rounded text-[9px] font-bold">{d}</span>)}</div>
                <div className="grid grid-cols-3 gap-2 mb-4">
                  ...
                  <div className="text-center bg-white rounded-lg p-2 border border-blue-100"><div className="text-sm font-black text-black">{u.deployments}</div><div className="text-[9px] font-bold text-black uppercase">Projects</div></div>
                  <div className="text-center bg-white rounded-lg p-2 border border-blue-100"><div className="text-sm font-black text-black">{u.activeProjects}</div><div className="text-[9px] font-bold text-black uppercase">Active</div></div>
                </div>
                <div className="flex items-center justify-between mb-3">
                  <div className="text-xs text-black font-medium">{u.city} | Est. {u.established}</div>
                  <div className="flex items-center gap-1"><Star size={11} className="text-orange-400 fill-orange-400"/><span className="text-xs font-black text-black">{u.rating}</span></div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={e=>{e.stopPropagation();dispatch({type:'ADD_TOAST',payload:{title:'MoU Request Sent',message:'MoU request sent to '+u.name,type:'success'}});}} className="flex-1 py-1.5 text-[11px] font-bold rounded bg-blue-600 text-white hover:bg-blue-600">Request MoU</button>
                </div>
              </div>
            </div>
          )) : cList.map(c=>(
            <div key={c.id} onClick={()=>setSel({...c,_type:'CSR'})} className="bg-white rounded-2xl border border-blue-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer">
              <div className="absolute top-0 left-0 right-0 h-1 bg-orange-500 rounded-t-2xl"/>
              <div className="p-5 pt-6 relative">
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-orange-50 border border-orange-200 flex items-center justify-center text-orange-700 font-black text-sm shrink-0">{c.name[0]}</div>
                  <div>
                    <div className="text-sm font-bold text-black leading-tight">{c.name}</div>
                    <div className="text-[10px] text-black">{c.company}</div>
                  </div>
                </div>
                <div className="text-[11px] font-bold text-black mb-3 leading-snug">{c.theme}</div>
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <div className="text-center bg-orange-50 rounded-lg p-2 border border-orange-100"><div className="text-sm font-black text-orange-700">{c.funding}</div><div className="text-[9px] font-bold text-orange-400 uppercase">Total CSR</div></div>
                  <div className="text-center bg-white rounded-lg p-2 border border-blue-100"><div className="text-sm font-black text-black">{c.projects}</div><div className="text-[9px] font-bold text-black uppercase">Projects</div></div>
                </div>
                <div className="flex flex-wrap gap-1 mb-4">{c.domains.slice(0,3).map(d=><span key={d} className="px-1.5 py-0.5 bg-orange-50 text-orange-700 border border-orange-100 rounded text-[9px] font-bold">{d}</span>)}</div>
                <div className="flex items-center gap-2">
                  <button onClick={e=>{e.stopPropagation();setSel({...c,_type:'CSR'});}} className="flex-1 py-1.5 text-[11px] font-bold rounded border border-orange-200 text-orange-700 bg-orange-50 hover:bg-orange-100">View Profile</button>
                  <button onClick={e=>{e.stopPropagation();dispatch({type:'ADD_TOAST',payload:{title:'Partnership Request',message:'Partnership request sent to '+c.name,type:'success'}});}} className="flex-1 py-1.5 text-[11px] font-bold rounded bg-blue-600 text-white hover:bg-blue-600">Partner Up</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {sel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-blue-600/40 backdrop-blur-sm" onClick={()=>setSel(null)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-blue-100 w-full max-w-xl mx-4 max-h-[90vh] overflow-auto" onClick={e=>e.stopPropagation()}>
            <div className={`p-6 border-b border-blue-100 relative ${sel._type==='UNIVERSITY'?'bg-blue-50':'bg-orange-50'}`}>
              <div className={`absolute top-0 left-0 right-0 h-1 ${sel._type==='UNIVERSITY'?'bg-blue-500':'bg-orange-500'}`}/>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black ${sel._type==='UNIVERSITY'?'bg-blue-100 text-blue-700 border-2 border-blue-300':'bg-orange-100 text-orange-700 border-2 border-orange-300'}`}>{sel.name[0]}</div>
                  <div>
                    <div className="text-xl font-black text-black">{sel.name}</div>
                    <div className="text-xs text-black">{sel._type==='UNIVERSITY' ? sel.type+' | '+sel.city : sel.company}</div>
                  </div>
                </div>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-white rounded-xl p-4 border border-blue-100">
                ...
                <div className="space-y-1">
                  {sel._type==='UNIVERSITY' && <div className="text-xs text-black">{sel.rank}</div>}
                  <div className="flex items-center gap-2 text-xs text-black"><Mail size={11}/> {sel._type==='UNIVERSITY'?sel.contact:sel.contact}</div>
                  <div className="flex items-center gap-2 text-xs text-black"><Phone size={11}/> {sel.phone}</div>
                  {sel._type==='UNIVERSITY' && <div className="text-xs text-black">Est. {sel.established} | {sel.students} Students</div>}
                  {sel._type==='CSR' && <div className="text-xs text-black">HQ: {sel.headquarters}</div>}
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                {sel._type==='UNIVERSITY'
                  ? [{l:'Readiness',v:sel.readiness+'%',c:'text-blue-700'},{l:'Past Projects',v:sel.deployments,c:'text-black'},{l:'Active',v:sel.activeProjects,c:'text-green-600'},{l:'Faculty',v:sel.faculty,c:'text-black'},{l:'Rating',v:'★ '+sel.rating,c:'text-orange-500'},{l:'City',v:sel.city,c:'text-black'}]
                  : [{l:'CSR Fund',v:sel.funding,c:'text-orange-700'},{l:'Range',v:sel.range,c:'text-black'},{l:'Total Projects',v:sel.projects,c:'text-black'},{l:'Active',v:sel.active,c:'text-green-600'},{l:'Theme',v:sel.theme.split('&')[0],c:'text-black'},{l:'HQ',v:sel.headquarters,c:'text-black'}]
                  .map(s=><div key={s.l} className="bg-white rounded-xl p-3 border border-blue-100 text-center"><div className={`text-base font-black ${s.c}`}>{s.v}</div><div className="text-[9px] font-bold text-black uppercase mt-1">{s.l}</div></div>)}
              </div>
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-black mb-2">{sel._type==='UNIVERSITY'?'Domain Expertise':'Focus Domains'}</div>
                <div className="flex flex-wrap gap-1.5">{(sel._type==='UNIVERSITY'?sel.domains:sel.domains).map(d=><span key={d} className={`px-2 py-1 rounded text-[10px] font-bold border ${sel._type==='UNIVERSITY'?'bg-blue-50 text-blue-700 border-blue-200':'bg-orange-50 text-orange-700 border-orange-200'}`}>{d}</span>)}</div>
              </div>
              {sel._type==='CSR' && (
                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-black mb-2">Eligible Activity Categories</div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-blue-100">
                <button onClick={()=>dispatch({type:'ADD_TOAST',payload:{title:'MoU Request Sent',message:sel.name+' notified of partnership interest.',type:'success'}})} className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-bold border ${sel._type==='UNIVERSITY'?'border-blue-200 text-blue-700 bg-blue-50 hover:bg-blue-100':'border-orange-200 text-orange-700 bg-orange-50 hover:bg-orange-100'}`}>{sel._type==='UNIVERSITY'?'Request MoU':'Partner Up'}</button>
                <button onClick={()=>dispatch({type:'START_WORKFLOW',payload:{action:'INVITE_UNIVERSITY',partnerId:sel.id}})} className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-bold bg-blue-600 text-white hover:bg-blue-600">Invite to Challenge</button>
              </div>
            </div>
          </div>
        </div>
