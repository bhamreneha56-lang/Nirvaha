...
export default function IndustryDashboard() {
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-purple-950 rounded-2xl p-8 text-white flex justify-between shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setActiveTab('opportunities')} className="px-6 py-2.5 bg-white/10 text-white border border-white/20 rounded-lg font-bold text-sm hover:bg-white/20 transition backdrop-blur-sm">Post a Challenge</button>
          </div>
        </div>
  ...
  const renderDiscover = () => (
    <div className="flex flex-col gap-6 p-4">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">AI Recommended High-Impact Projects</h2>
          <p className="text-sm text-slate-500">Matched using domain fit, CSR mandate alignment, funding capacity, and geographic target.</p>
        </div>
        <button className="text-sm font-bold text-slate-600 hover:text-slate-900">View All Projects →</button>
      </div>
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
          ...
          <h3 className="text-lg font-black text-slate-900 mb-2">AI-Powered Mine Safety & Hazard Warning System</h3>
          <p className="text-xs text-slate-500 font-bold mb-4 flex items-center gap-1">IIT (ISM) Dhanbad • Pilot Stage</p>
          <p className="text-sm text-slate-600 mb-6">Underground roof collapsing and gas leakage cause fatal mining accidents. Real-time multi-...</p>
          
          <div className="bg-slate-50 rounded-xl p-4 mb-6">
            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">WHY AI MATCHED THIS:</h4>
          ...
          <div className="flex gap-2 mb-6">
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">AI/ML</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">IoT</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">Computer Vision</span>
          </div>
          <div className="mt-auto border-t border-slate-100 pt-4 flex justify-between items-center mb-4">
            <span className="text-xs text-slate-500 font-medium">Funding Goal</span>
            <span className="font-black text-slate-900">₹35,00,000</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button className="py-2 border border-slate-200 text-slate-700 font-bold text-xs rounded-lg hover:bg-slate-50">View Details</button>
            ...
            <button className="py-2 border border-slate-200 text-slate-700 font-bold text-xs rounded-lg hover:bg-slate-50">Mentor</button>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
          ...
          <h3 className="text-lg font-black text-slate-900 mb-2">Solar Microgrid for Tribal Village Water Purification</h3>
          <p className="text-xs text-slate-500 font-bold mb-4 flex items-center gap-1">BIT Mesra, Ranchi • Testing Stage</p>
          <p className="text-sm text-slate-600 mb-6">Remote tribal villages in Ranchi district lack clean drinking water and electricity. Fluoride...</p>
          ...
          <div className="flex gap-2 mb-6">
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">IoT</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">CleanTech</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">Solar PV</span>
          </div>
          <div className="mt-auto border-t border-slate-100 pt-4 flex justify-between items-center mb-4">
            <span className="text-xs text-slate-500 font-medium">Funding Goal</span>
            <span className="font-black text-slate-900">₹22,50,000</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button className="py-2 border border-slate-200 text-slate-700 font-bold text-xs rounded-lg hover:bg-slate-50">View Details</button>
            <button className="py-2 border border-orange-500 text-orange-500 font-bold text-xs rounded-lg hover:bg-orange-50">Express Interest</button>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
          ...
          <h3 className="text-lg font-black text-slate-900 mb-2">Smart Drone Sprayer for Precision Paddy Agriculture</h3>
          <p className="text-xs text-slate-500 font-bold mb-4 flex items-center gap-1">Birsa Agricultural University, Kanke • Prototype</p>
          <p className="text-sm text-slate-600 mb-6">Pest infestation destroys up to 35% of paddy crops in Jharkhand. Manual spraying is labor intensive...</p>
          ...
          <div className="flex gap-2 mb-6">
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">Drones</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">AI/ML</span>
            <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">Hardware</span>
          </div>
          <div className="mt-auto border-t border-slate-100 pt-4 flex justify-between items-center mb-4">
            <span className="text-xs text-slate-500 font-medium">Funding Goal</span>
            <span className="font-black text-slate-900">₹18,00,000</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button className="py-2 border border-slate-200 text-slate-700 font-bold text-xs rounded-lg hover:bg-slate-50">View Details</button>
            <button className="py-2 border border-orange-500 text-orange-500 font-bold text-xs rounded-lg hover:bg-orange-50">Express Interest</button>
    ...
    <div className="flex flex-col gap-6 p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">Track milestone progression, fund releases, and project communication</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm min-h-[600px]">
        <h2 className="text-xl font-bold text-slate-900 mb-2">Active Collaborations & Project Monitoring</h2>
        <p className="text-sm text-slate-500 mb-6">Track real-time progress, escrow funding release, support cards, and multi-stakeholder chat.</p>
        
        <div className="flex border-b border-slate-200 mb-8">
          <button onClick={() => setCollabTab('active')} className={`px-6 py-3 font-bold text-sm border-b-2 transition ${collabTab === 'active' ? 'border-orange-500 text-orange-500' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>Active Projects (2)</button>
          <button onClick={() => setCollabTab('milestone')} className={`px-6 py-3 font-bold text-sm border-b-2 transition ${collabTab === 'milestone' ? 'border-orange-500 text-orange-500' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>Milestone Progression</button>
          <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500 hover:text-slate-800">Escrow Funding & Release</button>
          <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500 hover:text-slate-800">Support Programs</button>
          <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500 hover:text-slate-800">Project Communication</button>
        </div>
        ...
        {collabTab === 'active' ? (
          <div className="grid grid-cols-2 gap-6">
            <div className="border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Stage: Pilot Stage</span>
              </div>
              <h3 className="text-xl font-black text-slate-900 mb-1">AI Mine Safety & Hazard Warning System</h3>
              <p className="text-sm text-slate-500 mb-6">IIT (ISM) Dhanbad</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-slate-50 rounded-xl">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Committed Grant</div>
                  <div className="font-black text-slate-900">₹35,00,000</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Next Milestone Deadline</div>
            ...
            <div className="border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Stage: Testing Stage</span>
              </div>
              <h3 className="text-xl font-black text-slate-900 mb-1">Solar Microgrid for Tribal Villages</h3>
              <p className="text-sm text-slate-500 mb-6">BIT Mesra, Ranchi</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-slate-50 rounded-xl">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Committed Grant</div>
                  <div className="font-black text-slate-900">₹22,50,000</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Next Milestone Deadline</div>
          ...
          <div className="bg-slate-50 rounded-2xl p-8 border border-slate-200">
             <div className="mb-10">
               ...
               <h3 className="text-xl font-black text-slate-900">AI Mine Safety & Hazard Warning System</h3>
               <p className="text-sm text-slate-500">IIT (ISM) Dhanbad</p>
             </div>

             <div className="relative flex justify-between mb-16">
                <div className="absolute top-6 left-12 right-12 h-1 bg-slate-200 -z-10"></div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="font-bold text-sm text-slate-900">Prototype Hardware</span>
                  <span className="text-xs font-bold text-slate-500">₹10,00,000</span>
                </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="font-bold text-sm text-slate-900">Lab Simulation & Testing</span>
                  <span className="text-xs font-bold text-slate-500">₹15,00,000</span>
                </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="w-12 h-12 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center text-slate-400 font-black mb-3 shadow-sm">4</div>
                  <span className="font-bold text-sm text-slate-500">Final Safety Certification</span>
                  <span className="text-xs font-bold text-slate-400">₹5,00,000</span>
                </div>
             </div>
             ...
             <div className="flex gap-6">
                <div className="flex-1 bg-white border border-slate-200 rounded-xl p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="font-black text-lg text-slate-900">Prototype Hardware <span className="text-sm font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded ml-2">Approved</span></h4>
                  </div>
                  <p className="text-sm text-slate-600 mb-4">IoT sensor mesh fabricated & bench tested.</p>
                ...
                <div className="flex-1 bg-green-50 border border-green-200 rounded-xl p-6 flex items-start gap-4">
                   ...
                   <div>
                     ...
                     <div className="text-sm font-bold text-slate-800 leading-relaxed">APPROVED! Escrow release of ₹10,00,000 initiated to university account.</div>
                   </div>
                </div>
             </div>
          </div>
  ...
  const renderOpportunities = () => (
    <div className="flex flex-col gap-6 p-4">
      <div className="flex border-b border-slate-200 mb-2">
        ...
        <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500">Startup Launchpad (2)</button>
        <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500">Investor Pitch Days (2)</button>
      </div>

      <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 flex justify-between items-center">
         <div>
           <h2 className="text-lg font-black text-slate-900 mb-1">Earn Honorarium & Contribute Senior Technical Guidance</h2>
           <p className="text-sm text-slate-600">Short 2-5 hour reviews requested by university research groups across Jharkhand.</p>
         </div>
      ...
      <div className="grid grid-cols-3 gap-6">
         <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs text-slate-500 font-semibold flex items-center gap-1"><Clock size={12}/> 3 Hours</span>
            </div>
         ...
         <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs text-slate-500 font-semibold flex items-center gap-1"><Clock size={12}/> 4 Hours</span>
            </div>
         ...
         <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs text-slate-500 font-semibold flex items-center gap-1"><Clock size={12}/> 2 Hours</span>
            </div>
  ...
  const renderAnalytics = () => (
    <div className="flex flex-col gap-6 p-4">
      <div className="mb-2">
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">CSR Impact, Compliance & Analytics</h1>
        <p className="text-sm text-slate-500 font-medium">Audit CSR investments, verify digital certificates, track SDG alignment, and view state leaderboards.</p>
      </div>
      ...
      <div className="flex border-b border-slate-200 mb-4">
        ...
        <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500">Verified CSR Certificates (2)</button>
        <button className="px-6 py-3 font-bold text-sm border-b-2 border-transparent text-slate-500">Social Impact & Leaderboard</button>
      </div>

      <div className="grid grid-cols-5 gap-4 mb-2">
        <div className="bg-slate-100 border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-slate-500">Funding Committed</span><span className="text-green-500 font-bold bg-green-50 px-1 rounded">$</span></div>
          <div className="text-2xl font-black text-slate-900 mb-2">₹1.45 Cr</div>
          <span className="text-[10px] font-bold text-slate-400">+18% YoY</span>
        </div>
        <div className="bg-slate-100 border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-slate-500">Projects Supported</span><Building size={16} className="text-indigo-500 bg-indigo-50 px-1 rounded" /></div>
          <div className="text-2xl font-black text-slate-900 mb-2">14</div>
          <span className="text-[10px] font-bold text-slate-400">5 Universities</span>
        </div>
        <div className="bg-slate-100 border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-slate-500">Mentorship Hours</span><Users size={16} className="text-purple-500 bg-purple-50 px-1 rounded" /></div>
          <div className="text-2xl font-black text-slate-900 mb-2">240 Hrs</div>
          <span className="text-[10px] font-bold text-slate-400">32 Engineers</span>
        </div>
        <div className="bg-slate-100 border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-slate-500">Deployments</span><CheckCircle2 size={16} className="text-orange-500 bg-orange-50 px-1 rounded" /></div>
          <div className="text-2xl font-black text-slate-900 mb-2">6 Pits/Sites</div>
          <span className="text-[10px] font-bold text-slate-400">Field Active</span>
        </div>
        <div className="bg-slate-100 border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold text-slate-500">Impact Score</span><Award size={16} className="text-orange-500 bg-orange-50 px-1 rounded" /></div>
          <div className="text-2xl font-black text-slate-900 mb-2">94.8 / 100</div>
          <span className="text-[10px] font-bold text-slate-400">Top 5% in State</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
         <div className="bg-slate-100 border border-slate-200 rounded-2xl p-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="font-bold text-slate-900">Funding Disbursed Over Time</h3>
                <p className="text-xs text-slate-500">Quarterly CSR Escrow Payouts (2025 - 2026)</p>
              </div>
              <span className="text-xs font-bold text-green-500">+24% Growth</span>
            </div>
            <div className="flex items-end gap-4 h-48 mt-4 pt-4 border-t border-slate-200">
               <div className="flex flex-col items-center flex-1"><span className="text-xs font-bold mb-1">₹20L</span><div className="w-full bg-slate-800 rounded-t-sm h-[20%]"></div><span className="text-xs mt-2">Q1 2025</span></div>
               <div className="flex flex-col items-center flex-1"><span className="text-xs font-bold mb-1">₹35L</span><div className="w-full bg-slate-800 rounded-t-sm h-[35%]"></div><span className="text-xs mt-2">Q2 2025</span></div>
               <div className="flex flex-col items-center flex-1"><span className="text-xs font-bold mb-1">₹50L</span><div className="w-full bg-slate-800 rounded-t-sm h-[50%]"></div><span className="text-xs mt-2">Q3 2025</span></div>
               <div className="flex flex-col items-center flex-1"><span className="text-xs font-bold mb-1">₹75L</span><div className="w-full bg-slate-800 rounded-t-sm h-[75%]"></div><span className="text-xs mt-2">Q4 2025</span></div>
               <div className="flex flex-col items-center flex-1"><span className="text-xs font-bold mb-1">₹1.1Cr</span><div className="w-full bg-slate-800 rounded-t-sm h-[90%]"></div><span className="text-xs mt-2">Q1 2026</span></div>
         ...
         <div className="bg-slate-100 border border-slate-200 rounded-2xl p-6">
            <h3 className="font-bold text-slate-900">Projects by Industry Sector</h3>
            <p className="text-xs text-slate-500 mb-6">Domain distribution</p>
            
            <div className="space-y-6">
              <div>
                <div className="flex justify-between text-sm font-bold text-slate-700 mb-2"><span>Mining & Heavy Industry</span><span>6 Projects (40%)</span></div>
                <div className="h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-orange-500 w-[40%]"></div></div>
              </div>
              <div>
                <div className="flex justify-between text-sm font-bold text-slate-700 mb-2"><span>Rural Development & Clean Water</span><span>4 Projects (25%)</span></div>
                <div className="h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[25%]"></div></div>
              </div>
              <div>
                <div className="flex justify-between text-sm font-bold text-slate-700 mb-2"><span>Agritech & Precision Farming</span><span>3 Projects (20%)</span></div>
                <div className="h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-green-500 w-[20%]"></div></div>
              </div>
              <div>
                <div className="flex justify-between text-sm font-bold text-slate-700 mb-2"><span>Fintech & Blockchain Governance</span><span>1 Project (15%)</span></div>
                <div className="h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-purple-500 w-[15%]"></div></div>
              </div>
            </div>
         </div>
      </div>
  ...
  const renderOrganisation = () => (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[calc(100vh-160px)] p-8 m-4">
       <div className="flex justify-between items-start mb-8 pb-8 border-b border-slate-100">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Tata Steel Foundation</h2>
            <p className="text-slate-500 font-medium mt-1">Corporate Social Responsibility Partner</p>
          </div>
       ...
       <div className="grid grid-cols-3 gap-12">
          <div className="col-span-2">
             ...
             <p className="text-slate-600 leading-relaxed mb-6">Tata Steel Foundation is the CSR arm of Tata Steel, committed to sustainable development and inclusive growth across Jharkhand. We actively collaborate with technical universities to fund and mentor high-impact solutions in mining safety, rural electrification, and water resource management.</p>
             <h3 className="font-bold text-slate-900 mb-4">CSR Focus Areas</h3>
             <div className="flex flex-wrap gap-2">
               <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded font-semibold text-sm">Mining Safety & IoT</span>
               <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded font-semibold text-sm">Rural Development</span>
               <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded font-semibold text-sm">Clean Water Tech</span>
               <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded font-semibold text-sm">Skill Development</span>
             </div>
          </div>
          <div>
             <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                <h4 className="font-bold text-slate-900 mb-4">Contact & Admin</h4>
                <div className="space-y-4 text-sm">
                  <div><div className="text-slate-500 font-medium text-xs mb-1">Primary CSR Head</div><div className="font-bold text-slate-800">Dr. Soumitra Singh</div></div>
                  <div><div className="text-slate-500 font-medium text-xs mb-1">Email</div><div className="font-bold text-slate-800">csr@tatasteelfoundation.org</div></div>
                  <div><div className="text-slate-500 font-medium text-xs mb-1">Registration</div><div className="font-bold text-slate-800">CSR-2019-JH-9982</div></div>
                  <div><div className="text-slate-500 font-medium text-xs mb-1">Annual CSR Budget</div><div className="font-bold text-slate-800">₹12.5 Cr</div></div>
                </div>
                <button className="w-full mt-6 py-2 bg-slate-900 text-white rounded-lg font-bold hover:bg-black transition">Edit Profile</button>
             </div>
          </div>
       </div>
    </div>
