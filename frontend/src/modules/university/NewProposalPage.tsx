...
const AI_DRAFT: Record<string, string> = {
  ...
  innovationAngle: "Unlike conventional centralized water treatment requiring grid power, our solution uses hybrid solar-battery operation with offline-capable IoT monitoring via LoRaWAN mesh networking — eliminating dependence on grid power and internet connectivity. The modular design allows expansion without redesign. A QR-code-linked maintenance log ensures traceability of filter replacements.",
  ...
  testingPlan: "Month 1: Lab validation of filtration efficiency (target: arsenic <10 ppb, fluoride <1.5 mg/L per BIS 10500). Month 3: Pilot village water quality test by JTRL (independent). Month 5: Community satisfaction survey. Month 6: NABL-accredited lab certification before full scale-up.",
};
...
export default function ProposalPage() {
  ...
  if (submitted) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <h2 className="text-2xl font-black text-slate-900">Proposal Submitted!</h2>
      <p className="text-slate-500 mt-2 text-sm">Your proposal for <span className="font-semibold">{problem.title}</span> has been submitted to the government for review.</p>
      <p className="text-xs text-slate-400 mt-1">Expected review time: 5-7 working days</p>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900">New Proposal</h1>
          <p className="text-sm text-slate-500 mt-0.5">Submit your university's solution proposal</p>
        </div>
        <button onClick={() => setAiModal(true)}
          className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-300">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        <label className="text-sm font-bold text-slate-700 block mb-2">Challenge</label>
        ...
        {problem && <p className="text-xs text-slate-400 mt-2">Domain: {problem.domain} · Deadline: {problem.deadline} · Funding: {problem.fundingAvailable}</p>}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-5">
        {fields.map(f => (
          <div key={f.key}>
            <label className="text-sm font-bold text-slate-700 block mb-1.5">{f.label}</label>
            <textarea rows={f.rows} value={form[f.key] || ""} onChange={e => setForm(prev => ({...prev,[f.key]:e.target.value}))}
              placeholder={f.placeholder}
              className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none leading-relaxed"/>
          </div>
        ))}

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-bold text-slate-700 block mb-1.5">Cost Estimate (₹)</label>
            <input type="number" value={form.costEstimate} onChange={e => setForm(p=>({...p,costEstimate:e.target.value}))}
              placeholder="e.g. 1500000" className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-300"/>
          </div>
          <div>
            <label className="text-sm font-bold text-slate-700 block mb-1.5">Project Timeline</label>
            <input type="text" value={form.timeline} onChange={e => setForm(p=>({...p,timeline:e.target.value}))}
              placeholder="e.g. April 2026 – October 2026" className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-300"/>
          </div>
        </div>

        <div>
          <label className="text-sm font-bold text-slate-700 block mb-1.5">Resources Needed</label>
          <textarea rows={2} value={form.resources || ""} onChange={e => setForm(p=>({...p,resources:e.target.value}))}
            placeholder="Equipment, personnel, software, field access..." className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none"/>
        </div>
      </div>

      <div className="flex gap-3 justify-end">
        <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-slate-300 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setAiModal(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center"><Zap size={18} className="fill-white text-white"/></div>
              <div>
                <h3 className="font-black text-slate-900">AI Writing Assistant</h3>
                <p className="text-xs text-slate-500">Pre-filled based on {problem.title}</p>
              </div>
            </div>
            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-4 max-h-64 overflow-y-auto space-y-3">
              {fields.map(f => (
                <div key={f.key}>
                  <p className="text-xs font-bold text-indigo-700">{f.label}</p>
                  <p className="text-xs text-slate-600 mt-0.5 leading-relaxed">{AI_DRAFT[f.key]?.slice(0,120)}...</p>
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setAiModal(false)} className="flex-1 py-2.5 rounded-xl border border-slate-300 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</button>
              <button onClick={applyAIDraft} className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 flex items-center justify-center gap-2">
